...
# Licensed under the MIT License.
# Written by Weijia Wu (weijiawu@zju.edu.cn)
# ------------------------------------------------------------------------------
...
from datadm.pose_transforms import get_affine_transform
from datadm.pose_transforms import affine_transform
from datadm.pose_transforms import fliplr_joints
...
class JointsDataset(Dataset):
    def __init__(self, cfg, root, is_train, transform=None, coord_representation='heatmap', simdr_split_ratio=1):
        self.num_joints = 0
        self.pixel_std = 200
        self.flip_pairs = []
        self.parent_ids = []
...
#         cv2.imwrite("./test.png",input)
        ...
        if self.coord_representation == 'simdr':
            ...
            input = (input / 127.5 - 1.0).astype(np.float32)
            input = torch.from_numpy(input).permute(2, 0, 1)
            prompt = "a photo of a person"
            return input, filtered_joints[:,0:2], target_weight, meta, prompt,original_image
