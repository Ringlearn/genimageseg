...
class PoseModule(nn.Module):
    ...
    def __init__(self,cfg
        ...
        self.mlp_head_x = nn.Linear(cfg.MODEL.HEAD_INPUT, int(cfg.DATASETS.IMAGE_SIZE[0]))
        self.mlp_head_y = nn.Linear(cfg.MODEL.HEAD_INPUT, int(cfg.DATASETS.IMAGE_SIZE[1]))
...
#         final_image_feature=F.interpolate(image_feature, size=512, mode='bilinear', align_corners=False)
        ...
        image_feature = rearrange(image_feature, 'b c h w -> b c (h w)')
        # torch.Size([1, 17, 1024])
        
        pred_x = self.mlp_head_x(image_feature)
        pred_y = self.mlp_head_y(image_feature)
