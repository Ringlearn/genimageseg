...
from train.train_pose_coco_heatmap import get_max_preds,plot_pose
import yaml
from train.train_instance_coco import dict2obj
...
#                         "photo of a {} is in on the mountain, {}","photo of a {} is in on the mountain, {}",
                        "photo of a {} is crossing a road, {}",
#                     "photo of a {} is sitting, {}"
                   ]
...
#             prompts = [choice(sub_classes_list)]
            prompts = ["{}, full-length photograph".format(choice(sub_classes_list))]
            ...
            for n in range(coords.shape[0]):
                for p in range(coords.shape[1]):
                    ...
                    if 1 < px < heatmap_width-1 and 1 < py < heatmap_height-1:
                        ...
                        str_cont=str_cont+ str(coords[n][p][0]) + ","
                        str_cont=str_cont+ str(coords[n][p][1]) + ","
                        str_cont=str_cont+ str(maxvals[n][p][0]) + ","
            ...
            str_cont = str_cont[:-1] + "\n"
            with open("{}/{}.txt".format(Mask_path,seed),'w') as f:    
                f.writelines([str_cont])
