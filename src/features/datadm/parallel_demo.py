...
from data import *
from utils.utils import register_all_ade20k_full
from train.datadm.train_instance_coco import dict2obj,instance_inference
from parallel_generate_Semantic_CityScapes_AnyClass import semantic_inference,semantic_inference_huamn
from train.datadm.train_pose_coco_heatmap import get_max_preds,plot_pose
...
##Test-Step3 Single process function
def sub_processor(pid , opt):
    ##Test-Step3.1 Load config files
    ## v1-inference.yaml is the config file using here, which is the built in inference cofig file in stable diffusion v1-4 
    ...
    ##Test-Step3.2 Load tockenizer from stable diffusion
    ## CLIPTokenizer is a built in module from tranformers lib
    ## tokenizer is transformers.models.clip.tokenization_clip.CLIPTokenizer object
    tokenizer = CLIPTokenizer.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="tokenizer")
    
    ##Test-Step3.3 Load VAE from stable diffusion
    ##AutoencoderKL is built in module in diffusers lib
    #VAE 
    ## vae is a diffusers.models.autoencoder_kl.AutoencoderKL object
    vae = AutoencoderKL.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="vae")
    freeze_params(vae.parameters())
    vae=vae.to(device)
    vae.eval()
    
    ##Test-Step3.4 Load UNET from stable diffusion
    ## UNet2D is the custumised module of this rep
    #UNet2DConditionModel UNet2D
    ## unet is a model.unet.UNet2D object
    unet = UNet2D.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="unet")
    freeze_params(unet.parameters())
    unet=unet.to(device)
    unet.eval()
    
    ##Test-Step3.5 Load text encoder from stable diffusion
    ## CLIPTextModel is the built in module from tranformers
    ## text_encoder is a transformers.models.clip.modeling_clip.CLIPTextModel object
    text_encoder = CLIPTextModel.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="unet")
    freeze_params(text_encoder.parameters())
    text_encoder=text_encoder.to(device)
    text_encoder.eval()

    ##Test-Step3.6 Load scheduler from stable diffusion
    ## StableDiffusionPipeline is the built in module from diffusers
    ## scheduler is a diffusers.schedulers.scheduling_pndm.PNDMScheduler object
    ...
    ##Test-Step3.8 Load architecture of custumized depth model
    # depth
    ...
    ##Test-Step3.9 Load architecture of custumized semantic segmentaion model
    # semantic
    ## seg_semantic_model is a model.segment.transformer_decoder.seg_decorder object
    ...
    ##Test-Step3.10 Load architecture of custumized instance segmentation model
    # instance
    ...
    ##Test-Step3.11 Load architecture of custumized open world segmentation model
    ...
    ##Test-Step3.12 Load architecture of custumized pose segmentation model
    ...
    ##Test-Step3.13 Load architecture of custumized pose estimation model
    # pose estimation
    pose_module=PoseModule_HeatMap(cfg_pose).to(device)
    
    ##Test-Step 3.14 Load parameetrs of each decoder models
    ## base_weights is a collections.OrderedDict instance
    ...
    ##Test-Step3.15 Create a Controller for controlling the attention layers
    ## controller is a train.AttentionStore object
    ...
    ##Test-Step 3.16 Start the model inferencings
    with torch.no_grad():
        
        for n in range(number_per_thread_num):
#           ##Test-Step3.17 Reset the diffusion feature dictionary and attention controller map
...
#             seed = 107474
            ...
            ##Test-Step3.18 Input prompt for inferenecing
            prompts = [opt.prompt]
            ...
            ##Test-Step3.19 Generate the output image from the loaded model and input prompt
            ...
            ##Test-Step3.20 Get the diffusion feature maps from loaded UNET archtecture
            diffusion_features=get_feature_dic()          
            
            ##Test-Step3.21 predict the depth map from depth decoder model by giving diffusion feature as input
            # depth
            ...
            ##Test-Step3.22 predict the instance segmentation map from segmentation decoder model by giving diffusion feature as input
            #instance
            ...
            ##Test-Step3.23 Create text embeddings from texts in open world file  
            ...
            ##Test-Step3.24 predict the open word segmentation map from segmentation decoder model 
            # by giving diffusion feature and  text encoder as input
            ...
            ##Test-Step3.25 predict the semantic segmentation map from segmentation decoder model by giving diffusion feature as input
            #semantic  cityscapes
            ...
            ##Test-Step3.26 predict the human pose segmentation map from pose decoder model by giving diffusion feature as input
            ...
            ##Test-Step3.27 predict the human pose estimation points  from pose decoder model by giving diffusion feature as input
            # pose estimation
...
##block7 Main function  
def main():
    ##Test-Step1 Argument parser defenitions
    ...
    parser.add_argument(
        ...
        type=str,
        help="uses prompt",
        default="./data/Prompts_From_GPT/cityscapes"
    )
    parser.add_argument(
        "--fixed_code",
        action='store_true',
    ...
    parser.add_argument(
        "--config_semantic",
        type=str,
        default="configs/model/stable-diffusion/v1-inference.yaml",
...
#     register_all_ade20k_full(_root)
    ...
    for i in range(opt.thread_num):
        ##Test-Step3 Single process
        p = mp.Process(target=sub_processor, args=(i, opt))
        p.start()
        processes.append(p)

    ##Test-Step4 Join the process
    for p in processes:
        p.join()
