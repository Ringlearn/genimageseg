...
def sub_processor(opt,temp_prompt):
...
#         #read prompt txt for each class 
    ...
    sub_classes_list=[temp_prompt]
    print("sub_classes_list ", sub_classes_list)    
    with torch.no_grad():


        # for n in range(number_per_thread_num):
        for eachprompt in sub_classes_list:
...
#                 prompts = ["a photo of a {}".format(class_target)]
            ...
            UI_image1_path="{}/{}_{}.jpg".format(Image_path,img_name,seed)
            UI_image2_path=visualise_segmentation()
...
#                 cv2.imwrite("{}/{}_{}_att.png".format(Mask_path,trainclass,seed), attention_maps_3211*255)
            seed+=1
            return [UI_image1_path,UI_image2_path]
          
    
def feature_extract(prompt):
    ...
    parser.add_argument(
        ...
        action='store_true',
        help="uses prompt",
        default="./data/Prompts_From_GPT/voc2012"
    )
    parser.add_argument(
        "--ddim_eta",
        type=float,
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/model/stable-diffusion/v1-inference.yaml",
...
# !python parallel_generate_Semantic_VOC_AnyClass.py --grounding_ckpt 'model/VOC2012_100RealImage.pth' --outdir './DataDiffusion/DataDMRepoPrompts/' --config config/dataset/VOC/VOC_Sematic_Seg_5Images.yaml
    opt = parser.parse_args()
    setattr(opt, "grounding_ckpt",'model/VOC2012_100RealImage.pth')
    setattr(opt, "outdir",'./DataDiffusion/DataDMRepoPrompts/')
    setattr(opt, "config", "config/dataset/VOC/VOC_Sematic_Seg_5Images.yaml")
...
#     per_thread_video_num = int(len(coco_category_list)/thread_num)
    # thread_num=1
    images_paths = sub_processor(opt,prompt)
    return images_paths
...
def datasetDM_func(prompt,show_segment):
    images=feature_extract(prompt)
    if show_segment=="no":
      return images[0]
    if show_segment=="yes":
...
if __name__ == "__main__":
    # main()
    images_paths=datasetDM_func("An image of a car","no")
    print(images_paths)
