...
from src.model.unet import UNet2D,get_feature_dic,clear_feature_dic
from src.model.segment.transformer_decoder import seg_decorder
from train.train_instance_coco import dict2obj,instance_inference
...
import yaml
from train.utils import mask_image,get_findContours
...
#     noise_scheduler = DDPMScheduler.from_config("CompVis/stable-diffusion-v1-4", subfolder="scheduler")
    ...
    try:
        ...
        for k, v in base_weights.items():
            name = k[7:]   # remove `vgg.`，即只取vgg.0.weights的后面几位  module.
...
#                     prompts = ["a photo of {}".format(choice(sub_classes_list))]
#                     prompts = ["a photo of a {}".format(class_target)]
                else:
                    prompts = ["a photo of {}".format(class_target)]
#                 prompts = ["a photo of two {}".format(class_target)]  
#                 prompts = ["a photo of a {}".format(class_target)]
                
#                 prompts = ["a photo of person, no background, high quality"]
        
                print("prompts:",prompts,class_target)
                ...
                for prompt_class in classes:
                    ...
                    query_text=classes[prompt_class]
                    print(query_text)
                    ...
                    for mask_cls_result, mask_pred_result in zip(mask_cls_results, mask_pred_results):
                        ...
                        if cfg.SEG_Decorder.task=="semantic":
                            ...
                            instance_r = retry_if_cuda_oom(instance_inference)(mask_cls_result, mask_pred_result, 80, 100)
                            ...
                            txt = []
                            vis_i  = images_here[0].astype(np.uint8)
                            vis_i = np.array(vis_i,dtype=float)

                            import heapq
                            topk_idx = heapq.nlargest(20, range(len(scores)), scores.__getitem__)
                            for idxx,(c,m) in enumerate(zip(pred_classes,pred_masks)):
                                if idxx not in topk_idx:
                                    continue
                                if scores[idxx]<0.4:
                                    continue
                                print(c)
...
#                                 str_cont=str_cont + str(prompt_class-1) + "\n"
                                str_cont=str_cont + str(c.cpu().numpy()) + "," + str(scores[idxx].cpu().numpy()) + "\n"
                                txts.append(str_cont)
#                                 vis_i,_ = mask_image(vis_i,m)

#                 full_arr = softmax(full_arr, axis=0)
#                 mask = np.argmax(full_arr, axis=0)
#                 label_pred_prob = vis_i
#                 cv2.imwrite("{}/{}_{}.png".format(Mask_path,trainclass,seed), label_pred_prob)
                with open("{}/{}_{}.txt".format(Mask_path,trainclass,seed),'w') as f:    #设置文件对象
                    f.writelines(txts)
    
#                 cv2.imwrite("{}/{}_{}.txt".format(Mask_path,trainclass,seed), label_pred_prob)
...
def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        help="uses prompt",
        default="./data/Prompts_From_GPT/coco"
    )
    parser.add_argument(
        "--fixed_code",
        action='store_true',
