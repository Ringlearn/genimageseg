...
from src.model.datadm.diffusers.models.unet_2d_condition import UNet2DConditionModel
...
from src.model.datadm.segment.criterion import SetCriterion
from src.model.datadm.segment.matcher import HungarianMatcher
from detectron2.modeling.postprocessing import sem_seg_postprocess
from detectron2.utils.memory import retry_if_cuda_oom
import yaml
from src.utils.utils import mask_image
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="?",
        default="config/dataset/datadm/cityscapes/Cityscapes_Semantic_Seg_50images.yaml",
...
#     opt.image_limitation = cfg.DATASETS.image_limitation
    ...
    trained_path = os.path.join(ckpt_dir, 'latest_checkpoint.pth')
    if os.path.exists(trained_path):
        base_weights = torch.load(trained_path, map_location="cpu")
            ...
            if step%100 ==0:# or losses['loss_mask']*cfg.SEG_Decorder.MASK_WEIGHT<0.3:
                ...
                for mask_cls_result, mask_pred_result in zip(mask_cls_results, mask_pred_results):
                    ...
                    if cfg.SEG_Decorder.task=="semantic":
                        ...
                        cv2.imwrite(os.path.join(ckpt_dir, 'samples/datadm/'+'viz_sample_{0:05d}_seg'.format(step)+'.png'),label_pred_prob*100)
...
#                         print(pred_masks.shape)
                        cv2.imwrite(os.path.join(ckpt_dir, 'samples/datadm/'+'viz_sample_{0:05d}_seg'.format(step)+'.jpg'),vis_i)
                        try:
                            gt_mask = instances["gt_masks"][0][0]
                            gt_mask = np.array(gt_mask)*255
                            cv2.imwrite(os.path.join(ckpt_dir, 'samples/datadm/'+'viz_sample_{0:05d}_gt_seg'.format(step)+'.jpg'),gt_mask)
                        except:
                            pass
