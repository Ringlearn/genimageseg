...
def splicing_1x2(root,out_root, image_list,n_image=100,start_id=0):
    ...
    for idx in tqdm(range(n_image)):
        ...
        if not os.path.exists("./DataDiffusion/{}/Annotation/{}".format(root,mask_1)) or not os.path.exists("./DataDiffusion/{}/Annotation/{}".format(root,mask_2)):
            continue
            
        img1 = cv2.imread("./DataDiffusion/{}/Image/{}".format(root,image_1))
        img2 = cv2.imread("./DataDiffusion/{}/Image/{}".format(root,image_2)) 

        mas1 = cv2.imread("./DataDiffusion/{}/Annotation/{}".format(root,mask_1))
        mas2 = cv2.imread("./DataDiffusion/{}/Annotation/{}".format(root,mask_2))
        ...
        cv2.imwrite("./DataDiffusion/{}/Annotation/splicing_{}.png".format(out_root,start_id),mask)
...
#     start_id = 0
    ...
    for idx in tqdm(range(n_image)):
        ...
        cv2.imwrite("./DataDiffusion/{}/Annotation/splicing_{}.png".format(out_root,start_id),list_mask_ha)
...
def main():
    ...
    os.makedirs("./DataDiffusion/{}/Annotation/".format(opt.out_root), exist_ok=True)
    ...
    for i in range(opt.thread_num):
        ...
        if i == 0:
            p = mp.Process(target=splicing_1x2, args=(opt.input_root,opt.out_root,image_list, 5000, 0))
            p.start()
            processes.append(p)
        elif i == 1:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,7000,15001,2))
            p.start()
            processes.append(p)
        elif i == 2:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,8000,35001,3))
            p.start()
            processes.append(p)
        elif i == 3:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,6000,55001,5))
            p.start()
            processes.append(p)
        elif i == 4:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,5000,65001,6))
            p.start()
            processes.append(p)
        elif i == 5:
