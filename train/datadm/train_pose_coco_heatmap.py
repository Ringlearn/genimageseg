...
from src.model.diffusers.models.unet_2d_condition import UNet2DConditionModel
from transformers import CLIPFeatureExtractor, CLIPTextModel, CLIPTokenizer
from src.model.unet import UNet2D,get_feature_dic,clear_feature_dic
from src.model.pose_module_heat import PoseModule_HeatMap
import torch.optim as optim
import torch.nn.functional as F
from src.model.segment.criterion import SetCriterion
from src.model.segment.matcher import HungarianMatcher
from detectron2.modeling.postprocessing import sem_seg_postprocess
from detectron2.utils.memory import retry_if_cuda_oom
import yaml
from src.model.loss import NMTCritierion,JointsMSELoss
from train.utils import mask_image
...
def plot_pose(image,dt_joints,maxvals):
    # Plot
    w = h = 512
    ref = 512
...
#     opt.image_limitation = cfg.DATASETS.image_limitation
    ...
    if opt.dataset == "VOC":
        ...
        dataset = COCODataset_Pose(
            cfg.DATASETS, is_train="train",image_limitation = opt.image_limitation
        ) 
        loss_fn = JointsMSELoss(use_target_weight = True).cuda()
    else:
        return
    ...
    pose_module=PoseModule_HeatMap(cfg).to(device)
...
#             print(outputs.shape,target.shape)
            total_loss = loss_fn(outputs, target, target_weight)
            g_optim.zero_grad()
            total_loss.backward()
            g_optim.step()
            ...
            if step%100 ==0:
                ...
                save_batch_heatmaps(original_image,outputs,os.path.join(ckpt_dir, 'training/pred_sample_{0:05d}_seg'.format(j)+'.png'))
                
                save_batch_heatmaps(original_image,target,os.path.join(ckpt_dir, 'training/GT_sample_{0:05d}_seg'.format(j)+'.png'))
