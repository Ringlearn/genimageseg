...
#     thread_num=8
    ...
    for i in range(opt.thread_num):
        ...
        if i == 0:
            p = mp.Process(target=splicing_1x2, args=(opt.input_root,opt.out_root,image_list, 2000, 0))
            p.start()
            processes.append(p)
        elif i == 1:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,3000,15001,2))
            p.start()
            processes.append(p)
        elif i == 2:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,3000,35001,3))
            p.start()
            processes.append(p)
        elif i == 3:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,2000,55001,5))
            p.start()
            processes.append(p)
        elif i == 4:
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,2000,65001,6))
            p.start()
            processes.append(p)
        elif i == 5:
            ...
            p = mp.Process(target=splicing_NxN, args=(opt.input_root,opt.out_root,image_list,500,85001,8))
            p.start()
            processes.append(p)
